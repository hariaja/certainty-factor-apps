...
namespace App\DataTables\Consoles\Settings;
...
class UserDataTable extends DataTable
{
  ...
  public function dataTable(QueryBuilder $query): EloquentDataTable
  {
    return (new EloquentDataTable($query))
      ->addIndexColumn()
      ->editColumn('roles', fn ($row) => $row->getRoleBadge())
      ->editColumn('status', fn ($row) => $row->statusLabel)
      ->addColumn('edit_status', 'consoles.settings.users.status')
      ->addColumn('action', 'consoles.settings.users.action')
      ->rawColumns([
        'roles',
        'status',
        'action',
